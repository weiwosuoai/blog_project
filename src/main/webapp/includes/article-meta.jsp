...
<div class="sub-article-meta">
	<span>发表于&nbsp;</span> <span class="sub-article-post-time"> <fmt:formatDate
		...
		<c:choose>
			<c:when test="${article.category == 1}"><a href="<%=contextPath%>/archive/java">Java</a></c:when>
			<c:when test="${article.category == 2}"><a href="<%=contextPath%>/archive/android">Android</a></c:when>
			<c:when test="${article.category == 3}"><a href="<%=contextPath%>/archive/db">DB</a></c:when>
		</c:choose> 
	</span>
	<%-- 用户登录后，才会显示编辑超链接 --%>
